...
Precision::Precision(Character* target, unsigned short nbr_of_rounds, bool is_applicable_at_every_turn, bool is_temporary, unsigned int effect_level):
...
Precision::~Precision()
{
	if (_is_temporary)	//si l'effet était temporaire, on l'annule
	{
		if (_is_applicable_at_every_turn)	//si il s'appliquait à chaque tour, on enlève tous les effets accumulés
			_target->subtractPrecision(_effect_level * _initial_nbr_of_rounds);
		else								//sinon, on enlève l'effet
			_target->subtractPrecision(_effect_level);
	}
}

void Precision::apply()
{
