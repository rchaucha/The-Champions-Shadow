...
AbstractVersion::MenuChoice PicVersion::displayMenu()
{
...

  {
    ...
    Lock lock(*_mutex);
    _sprites_to_draw["background"] = background_sprite;
    _texts_to_draw["play"] = play_text;
    _texts_to_draw["settings"] = settings_text;
    _texts_to_draw["quit"] = quit_text;
  }

  Lock lock(*_mutex);
  Text &play_text = _texts_to_draw["play"];
  Text &settings_text = _texts_to_draw["settings"];
  Text &quit_text = _texts_to_draw["quit"];
  lock.~Lock();
  ...
  {
    if (_isMouseOver(play_text) || _isMouseOver(settings_text) || _isMouseOver(quit_text))
    {
      _changeCursor(_CursorType::Hand);
      if (_isMouseOver(play_text))
      {
        settings_text.setFillColor(Color::White);
        play_text.setFillColor(Color(255, 128, 0));
      }
      else if (_isMouseOver(settings_text))
      {
        play_text.setFillColor(Color::White);
        quit_text.setFillColor(Color::White);
        settings_text.setFillColor(Color(255, 128, 0));
      }
      else if (_isMouseOver(quit_text))
      {
        settings_text.setFillColor(Color::White);
        quit_text.setFillColor(Color(255, 128, 0));
      }
    }
    else
    {
    ...
    while (_pollEvent(event))
    {
      ...
      case Event::MouseButtonReleased:
        if (event.mouseButton.button == Mouse::Left)
        {
          ...
          if (_isMouseOver(settings_text))
            return MenuChoice::Settings;

          if (_isMouseOver(quit_text))
...
CharacterFactory::CharacterType PicVersion::chooseCharacter()
{
...

  {
    ...
    Lock lock(*_mutex);
    _sprites_to_draw["background"] = background_sprite;
    _texts_to_draw["warrior"] = warrior_text;
    _texts_to_draw["cleric"] = cleric_text;
  }

  Lock lock(*_mutex);
  Sprite &background_sprite = _sprites_to_draw["background"];
  Text &warrior_text = _texts_to_draw["warrior"];
  Text &cleric_text = _texts_to_draw["cleric"];
  lock.~Lock();
  ...
  while (_window->isOpen())
  {
    if (_isMouseOver(warrior_text) || _isMouseOver(cleric_text) || _isMouseOver(warrior_sprite) || _isMouseOver(cleric_sprite))
    {
      _changeCursor(_CursorType::Hand);
      if (_isMouseOver(warrior_text) || _isMouseOver(warrior_sprite))
      {
        warrior_text.setFillColor(Color(255, 128, 0));
        cleric_text.setFillColor(Color::White);
      }
      else if (_isMouseOver(cleric_text) || _isMouseOver(cleric_sprite))
      {
        cleric_text.setFillColor(Color(255, 128, 0));
        warrior_text.setFillColor(Color::White);
      }
    }
    else
    ...
    while (_pollEvent(event))
    {
      ...
      case Event::MouseButtonReleased:
        if (event.mouseButton.button == Mouse::Left)
        {
          ...
          if (_isMouseOver(cleric_text) || _isMouseOver(cleric_sprite))
...
string PicVersion::choosePseudo()
{
...

  {
    ...
    Lock lock(*_mutex);
    _sprites_to_draw["text_box"] = text_box_sprite;
    _texts_to_draw["box"] = box_text;
    _texts_to_draw["ok"] = ok_text;
    _texts_to_draw["pseudo"] = pseudo_text;
  }

  Lock lock(*_mutex);
  Text& pseudo_text = _texts_to_draw["pseudo"];
  Text& ok_text = _texts_to_draw["ok"];
  lock.~Lock();
  ...
  {
    Lock lock(*_mutex);
    if (_isMouseOver(ok_text))
    {
      _changeCursor(_CursorType::Hand);
      ok_text.setFillColor(Color(255, 128, 0));
    }
    else
    {
      ok_text.setFillColor(Color::White);
    ...
    while (_pollEvent(event))
    {
      ...
      case Event::TextEntered:
      {
        ...
        {Lock lock(*_mutex);
        pseudo_text.setString(pseudo);
        }break;
      }
      case Event::MouseButtonReleased:
        _changeCursor(_CursorType::Standard);
        if (event.mouseButton.button == Mouse::Left && _isMouseOver(ok_text))
          return pseudo;
        break;
      default:
        break;
...
void PicVersion::_versusAnim(Character * const player, Character * const enemy)
{
  ...
  Texture vs_texture;
  {
    ...
    Lock lock(*_mutex);
    _texts_to_draw["player_name"] = player_name;
    _texts_to_draw["enemy_name"] = enemy_name;
    _sprites_to_draw["background"] = background_sprite;
  }

  Lock lock(*_mutex);
  Sprite& vs = _sprites_to_draw["vs"];
  Text& player_name = _texts_to_draw["player_name"];
  Text& enemy_name = _texts_to_draw["enemy_name"];
  Text& background = _texts_to_draw["background"];
  lock.~Lock();
...
void PicVersion::_initScene(Character * const player, Character * const enemy)
{
  ...
  Lock lock(*_mutex);
  _sprites_to_draw["health_bar1"] = health_bar1;
  _sprites_to_draw["health_bar2"] = health_bar2;
  _sprites_to_draw["mana_bar"] = mana_bar;
  _sprites_to_draw["empty_bar1"] = empty_bar1;
  _sprites_to_draw["empty_bar2"] = empty_bar2;
  _sprites_to_draw["empty_bar3"] = empty_bar3;
  _texts_to_draw["player_life"] = player_life;
  _texts_to_draw["enemy_life"] = enemy_life;
  _texts_to_draw["player_mana"] = player_mana;
  _sprites_to_draw["text_box"] = text_box_sprite;
  _texts_to_draw["text_box"] = text_box_txt;
  _sprites_to_draw["z_next_arrow"] = next_arrow;
  _sprites_to_draw["z_attack_selector"] = attack_selector;

  _texts_to_draw["player_name"].setPosition(health_bar1.getPosition().x + 3, _window_height * 0.05f);
  _texts_to_draw["enemy_name"].setPosition(health_bar2.getPosition().x + 3, _window_height * 0.05f);
  _texts_to_draw["player_name"].setCharacterSize(35);
  _texts_to_draw["enemy_name"].setCharacterSize(35);
}
...
void PicVersion::lifeDisplay(Character * const player, Character * const enemy)
{
  ...
  Lock lock(*_mutex);
  _sprites_to_draw["health_bar1"].setTextureRect(IntRect(8, 0, int(_sprites_to_draw["empty_bar1"].getTextureRect().width * ratio_player), _sprites_to_draw["empty_bar1"].getTextureRect().height));
  _sprites_to_draw["health_bar2"].setTextureRect(IntRect(8, 0, int(_sprites_to_draw["empty_bar1"].getTextureRect().width  * ratio_enemy), _sprites_to_draw["empty_bar1"].getTextureRect().height));

  _texts_to_draw["player_life"].setString(to_string(player->getLife()) + "/" + to_string(player->getMaxLife()));
  _texts_to_draw["enemy_life"].setString(to_string(enemy->getLife()) + "/" + to_string(enemy->getMaxLife()));
}
...
void PicVersion::manaDisplay(Character * const player)
{
  ...
  Lock lock(*_mutex);
  _sprites_to_draw["mana_bar"].setTextureRect(IntRect(8, 0, int(_sprites_to_draw["empty_bar1"].getTextureRect().width * ratio), _sprites_to_draw["empty_bar1"].getTextureRect().height));

  _texts_to_draw["player_mana"].setString(to_string(player->getMana()) + "/" + to_string(player->getMaxMana()));
}
...
unsigned short PicVersion::attackChoice(Character * const player, bool is_enemy_attainable)
{
  ...
  {
    {Lock lock(*_mutex);
    temp_text = _texts_to_draw["text_box"];
    }
    ...
    Lock lock(*_mutex);
    _texts_to_draw["attack" + to_string(i)] = temp_text;
  ...
  Lock lock(*_mutex);
  Sprite& attack_selector = _sprites_to_draw["z_attack_selector"];
  lock.~Lock();
        ...
        if (event.mouseButton.button == Mouse::Left)
        {
          for (Text const& t : attacks)
          {
            if (_isMouseOver(t))
            {
              ...
              for (unsigned int i = 0; i < attacks_list.size(); i++)
...
void PicVersion::_draw()
{
  while (_window->isOpen())
  {
    ...
    if (!_sprites_to_draw.empty())
      for (auto const& x : _sprites_to_draw)
        _window->draw(x.second);
    } 
    {Lock lock(*_mutex);
    if (!_texts_to_draw.empty())
      for (auto const& x : _texts_to_draw)
        _window->draw(x.second);
...
void PicVersion::_AnimationLauncher::_animationLaunch()
{
  ...
  Lock lock(*_pic_version->_mutex);

  _pic_version->_sprites_to_draw[_id] = _entity.getSprite();
  Sprite &entity_sprite = _pic_version->_sprites_to_draw[_id];
  ...
  do
  {
    if (!_entity.getRelativeCoordinatesVect().empty())
    {
      for (const auto& x : _entity.getRelativeCoordinatesVect())
      {
        ...
        IntRect rect = entity_sprite.getTextureRect();
...
void PicVersion::_printToTextBox(string str)
{
  ...
  {
    Lock lock(*_mutex);
    Text text = _texts_to_draw["text_box"];

    text.setString(str);

    float text_left_pos = text.getGlobalBounds().left,
      text_width = text.getGlobalBounds().width,
      box_left_pos = _sprites_to_draw["text_box"].getGlobalBounds().left,
      box_width = _sprites_to_draw["text_box"].getGlobalBounds().width;
    lock.~Lock();
  ...
  {
    Lock lock(*_mutex);
    _texts_to_draw["text_box"].setString(str);
    lock.~Lock();
    ...
    {
      Lock lock(*_mutex);
      if (_isMouseOver(_sprites_to_draw["z_next_arrow"]))
      {
        _changeCursor(_CursorType::Hand);
        _sprites_to_draw["z_next_arrow"].setTextureRect(IntRect(215, 487, 23, int(_sprites_to_draw["z_next_arrow"].getGlobalBounds().height)));
      }
      else
      {
        _sprites_to_draw["z_next_arrow"].setTextureRect(IntRect(171, 487, 23, int(_sprites_to_draw["z_next_arrow"].getGlobalBounds().height)));
        _changeCursor(_CursorType::Standard);
      }
      lock.~Lock();
      ...
      while (_pollEvent(event))
      {
        ...
        if ((event.type == Event::MouseButtonReleased && event.mouseButton.button == Mouse::Left && _isMouseOver(_sprites_to_draw["z_next_arrow"]))
  ...
  Lock lock(*_mutex);
  _texts_to_draw["text_box"].setString("");
}

int PicVersion::_getAttackId(const Text& t)
{
  Lock lock(*_mutex);
  int i = int((t.getPosition().y - _texts_to_draw["text_box"].getPosition().y) / 32);
  return i;
}
...
{
  Lock lock(*_mutex);
  Sprite& text_box = _sprites_to_draw["text_box"];
  lock.~Lock();

  int top = 0;

  if (text_box.getTextureRect().top == 144)
    top = 49;
  else
    top = 144;
